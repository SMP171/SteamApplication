...
namespace DataAccessLibrary
{
    public class ProductService
    {
        ...
        public List <users_products> SelectAllUsersProducts()
        { 

            List<users_products> userProduct = new List<users_products>();

            DbCommand command = SqlConncetionHelper.Connection.CreateCommand();

            command.CommandText = "select * from dbo.users_products where user_id=@user_id";
            DbDataReader reader = command.ExecuteReader();
            while (reader.Read())
            {
                userProduct.Add(new users_products
                {
                    up_id=(int)reader["up_id"],
